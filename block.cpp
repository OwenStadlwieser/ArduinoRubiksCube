...
void cube::transform(const char transformation, cube& Mcube){
	...
	if(transformation == 'R'){
		...
		//transformations for edges of down face
		for (int i = 0; i< 3; i++){
			tempCube[0][2][i] = Mcube.cubeMatrix[3][2][abs(i-2)];
		}
		for (int i = 0; i< 3; i++){
			tempCube[1][2][i] = Mcube.cubeMatrix[0][2][abs(i-2)];
		}
		for (int i = 0; i< 3; i++){
			tempCube[2][2][i] = Mcube.cubeMatrix[1][2][abs(i-2)];
		}
		for (int i = 0; i< 3; i++){
			tempCube[3][2][i] = Mcube.cubeMatrix[2][2][abs(i-2)];
		}
	}
